...
class NoteService:
    ...
    def list_visible(self, user_id: int) -> list[Note]:
        ...
        combine_ids = list({*direct_ids, *ids_by_label})
        shared = self.notes.list_by_ids(combine_ids)
    ...
    def update(self, user_id: int, note_id: int, payload: NoteUpdate) -> Note:
        ...
        if label_ids is not None:
            if note.owner_id != user_id:
                raise HTTPException(status_code=403, detail="No existe o no autorizado para editar")
